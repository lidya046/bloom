...
router.post('/', async (req, res) => {
    ...
    `, [req.user.id]);
    if (pending.rows[0]) return res.json({ order: pending.rows[0], existing: true });

    const inserted = await pool.query(`
        INSERT INTO coin_orders (user_id, telegram_id, username, first_name, package_id, coins, price, status)
    ...
    await pool.query(`UPDATE coin_orders SET order_code=$1 WHERE id=$2`, [order.order_code, order.id]);

    const buyer = req.user.username ? `@${req.user.username}` : (req.user.first_name || `ID ${req.user.telegram_id}`);
    const text = [
    ...
    res.json({ order: { ...order, order_code: order.order_code }, notified });
});

router.post('/telegram', async (req, res) => {
    if (!webhookAuthorized(req)) return res.status(401).json({ error: 'Unauthorized webhook' });

    const cq = req.body?.callback_query;
    if (!cq?.data?.startsWith('coin:')) return res.json({ ok: true });

    const parts = String(cq.data).split(':');
    const action = parts[1];
    const orderId = Number(parts[2]);
    if (!['approve', 'reject'].includes(action) || !Number.isInteger(orderId)) {
        await answerCallback(cq.id, 'Data order tidak valid');
        return res.json({ ok: true });
    }

    if (!isAdminTelegramId(cq.from?.id)) {
        await answerCallback(cq.id, 'Akses admin ditolak');
        return res.json({ ok: true });
    }
    ...
    try {
        ...
        const r = await client.query(`SELECT * FROM coin_orders WHERE id=$1 FOR UPDATE`, [orderId]);
        const order = r.rows[0];
        if (!order) {
            await client.query('ROLLBACK');
            await answerCallback(cq.id, 'Order tidak ditemukan');
            return res.json({ ok: true });
        }

        if (order.status !== 'pending') {
            await client.query('ROLLBACK');
            await answerCallback(cq.id, `Order sudah ${order.status}`);
            return res.json({ ok: true });
        }

        if (action === 'approve') {
            await client.query(`UPDATE users SET coins=coins+$1 WHERE id=$2`, [order.coins, order.user_id]);
        }
        ...
        const updated = await client.query(`
            ...
            WHERE id=$3
            RETURNING *
        `, [action === 'approve' ? 'approved' : 'rejected', cq.from.id, order.id]);
        ...
        const adminText = [
            ...
            '',
            `🧾 Order: ${order.order_code}`,
            `👤 User: ${order.username ? `@${order.username}` : (order.first_name || `ID ${order.telegram_id}`)}`,
            `📦 Paket: ${Number(order.coins).toLocaleString('id-ID')} Coins`,
            `💵 Harga: ${formatRupiah(order.price)}`,
            `📌 Status: ${statusText}`,
            `👮 Admin: ${cq.from.username ? `@${cq.from.username}` : cq.from.id}`
        ].join('\n');
        await editTelegramMessage(cq.message?.chat?.id, cq.message?.message_id, adminText);
        await answerCallback(cq.id, action === 'approve' ? 'Coins berhasil ditambahkan' : 'Order ditolak');

        if (action === 'approve') {
            await sendTelegram(order.telegram_id,
        ...
        await client.query('ROLLBACK');
        await answerCallback(cq.id, 'Terjadi error saat memproses order');
