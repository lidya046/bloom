...
router.post('/', async (req,res)=>{
  ...
  const receiverId = receiverTelegramId ? Number(receiverTelegramId) : null;
  if (receiverTelegramId && !Number.isInteger(receiverId)) return res.status(400).json({error:'Telegram ID penerima tidak valid'});
  ...
  try{
    ...
    const b=(await client.query(`INSERT INTO bouquets(sender_id,receiver_telegram_id,receiver_username,message,wrapping) VALUES($1,$2,$3,$4,$5) RETURNING *`,[u.id,receiverId,receiverUsername,message,wrapping])).rows[0];
    ...
    if (receiverId) {
      try { notification = await notifyTelegram(receiverId, b, u); } catch (e) { notification = {sent:false, reason:e.message}; }
    }
    res.json({bouquet:b,cost,xp:15,notification});
